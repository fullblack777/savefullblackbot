    ...
    <style>
        ...
        * {
        ...
        @keyframes glitch {
            ...
            95% { transform: skew(5deg, -3deg); text-shadow: 5px 0 #ff00ff, -5px 0 #00ffff; }
            96% { transform: skew(-5deg, 3deg); text-shadow: -5px 0 #ff00ff, 5px 0 #00ffff; }
            97% { transform: skew(0); }
        }
        ...
        function initParticles() {
            ...
            for (let i = 0; i < 100; i++) {
                particles.push({
                    ...
                    color: `rgba(${Math.random() > 0.5 ? '0,255,255' : '255,0,255'}, ${Math.random() * 0.5 + 0.2})`
                });
            }
        }
...
<head>
    ...
    <style>
        * {
        ...
        .badge-admin { background: #f00; color: #fff; box-shadow: 0 0 10px #f00; }
        .badge-permanent { background: #0f0; color: #000; }
        .badge-temporary { background: #ff0; color: #000; }
        .badge-credits { background: #f0f; color: #fff; }
...
<body>
    ...
    <div class="container">
        ...
        <div class="dashboard-grid">
            ...
            <div class="card">
                ...
                <form method="POST">
                    ...
                    <div class="form-group"><label>Cartões (numero|mes|ano|cvv)</label><textarea name="ggs" required></textarea></div>
            ...
            <!-- Preços dos Planos -->
            <div class="card">
                <div class="card-header"><span>💰</span><h2>PREÇOS DOS PLANOS</h2></div>
                <table class="users-table">
                    <thead><tr><th>TIPO</th><th>DETALHES</th><th>PREÇO</th></tr></thead>
                    <tbody>
                        <?php foreach ($plans as $plan): ?>
                        <tr>
                            <td><?php echo $plan['type'] == 'hours' ? '⏱️ HORAS' : '💰 CRÉDITOS'; ?></td>
                            <td><?php echo $plan['type'] == 'hours' ? $plan['hours'].'h' : $plan['credits'].' créditos'; ?></td>
                            <td>
                                <form method="POST" style="display:flex;gap:5px;">
                                    <input type="hidden" name="admin_action" value="update_plan_price">
                                    <input type="hidden" name="plan_id" value="<?php echo $plan['id']; ?>">
                                    <input type="number" name="price" step="0.01" value="<?php echo $plan['price']; ?>" style="width:80px;">
            ...
            <div class="card" style="grid-column: span 2;">
                ...
                <table class="users-table">
                    <thead><tr><th>BIN</th><th>DISPONÍVEIS</th><th>TOTAL</th><th>PREÇO</th><th>AÇÃO</th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="POST" style="display:flex;gap:5px;">
                                    ...
                                    <input type="number" name="price" step="0.01" value="<?php echo $bin['price']; ?>" style="width:70px;">
                                    <button type="submit" class="action-btn">OK</button>
                                </form>
                            </td>
                        </tr>
        ...
        <div class="card" style="margin-top:30px;">
            ...
            <table class="users-table">
                <thead><tr><th>USUÁRIO</th><th>TIPO</th><th>💰</th><th>🪙</th><th>EXPIRA</th><th>✅</th><th>📊</th><th>AÇÕES</th></tr></thead>
                <tbody>
                    <?php foreach ($users as $username => $data): ?>
                    <tr>
                        <td><?php echo $username; ?> <?php if($data['role']==='admin') echo '<span class="badge badge-admin">ADMIN</span>'; ?></td>
                        ...
                        <td><?php echo ($data['type']==='temporary' && $data['expires_at']) ? date('d/m H:i',strtotime($data['expires_at'])) : '-'; ?></td>
                        ...
                        <td>
                            ...
                            <form method="POST" style="display:inline;">
                                ...
                                <button type="submit" class="action-btn" onclick="return confirm('Remover?')">🗑️</button>
                            </form>
                            <?php endif; ?>
                        </td>
                    </tr>
    ...
    <div class="modal" id="editModal">
        <div class="modal-content">
            <div class="modal-header"><h2>EDITAR USUÁRIO</h2><div class="modal-close" onclick="closeEditModal()">✖</div></div>
            <div id="modalUserInfo" style="margin-bottom:20px;"></div>
            <form method="POST" id="editForm">
                <input type="hidden" name="admin_action" id="editAction">
                <input type="hidden" name="username" id="editUsername">
                <div class="form-group"><label>Adicionar Créditos</label><input type="number" name="credits" step="0.01"></div>
                <div class="form-group"><label>Adicionar Moedas Cyber</label><input type="number" name="cyber_money" step="0.01"></div>
                <div class="form-group"><label>Adicionar Horas</label><input type="number" name="hours"></div>
                <div style="display:flex;gap:10px;">
                    <button type="button" class="btn-submit" onclick="submitEdit('add_credits')">💰</button>
                    <button type="button" class="btn-submit" onclick="submitEdit('add_cyber')">🪙</button>
                    <button type="button" class="btn-submit" onclick="submitEdit('extend_hours')">⏱️</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        // Canvas de partículas (reutilizar)
        const canvas = document.getElementById('particle-canvas');
        const ctx = canvas.getContext('2d');
        ...
        function openEditModal(username, credits, cyber, type) {
            document.getElementById('modalUserInfo').innerHTML = `<p><strong>${username}</strong> | 💰 ${credits.toFixed(2)} | 🪙 ${cyber.toFixed(2)} | Tipo: ${type}</p>`;
            document.getElementById('editUsername').value = username;
            document.getElementById('editModal').classList.add('active');
        }
        function closeEditModal() { document.getElementById('editModal').classList.remove('active'); }
        function submitEdit(action) { document.getElementById('editAction').value = action; document.getElementById('editForm').submit(); }
...
<body>
    ...
    <div class="container">
        <div class="header">
            <div>
                <h1>🛒 LOJA CYBERSEC</h1>
                <div style="color: #f0f;">👤 <?php echo $_SESSION['username']; ?></div>
            </div>
            <div class="balance"><span>🪙</span><?php echo number_format($user_cyber,2); ?></div>
        </div>
        ...
        <?php if(isset($_GET['mycards'])): ?>
            <div style="background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid #f0f; border-radius:40px; padding:30px;">
                <h2 style="color:#0ff; margin-bottom:20px;">📋 MEUS CARTÕES</h2>
                <?php if(empty($purchased_cards)): ?>
                    <div style="text-align:center; padding:50px;"><div style="font-size:64px;">📭</div><h3>Nenhum cartão comprado</h3></div>
                <?php else: ?>
                <table style="width:100%; border-collapse:collapse;">
                    <thead><tr style="background:rgba(255,0,255,0.2); color:#f0f;"><th>DATA</th><th>BIN</th><th>CARTÃO</th><th>VALIDADE</th><th>CVV</th><th>PREÇO</th></tr></thead>
                    <tbody>
        ...
        <?php else: ?>
            <h2 style="color:#f0f; margin-bottom:20px;">💰 ADQUIRA ACESSO</h2>
            <div class="plans-grid">
                ...
                <div class="plan-card">
                    ...
                    <div style="color:#0ff;">Entre em contato</div>
                </div>
                <?php endforeach; ?>
            </div>

            <h2 style="color:#0ff; margin:40px 0 20px;">💳 GGS DISPONÍVEIS</h2>
            <?php if(empty($ggs_by_bin)): ?>
                <div style="text-align:center; padding:50px; background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border-radius:40px;"><div style="font-size:64px;">📭</div><h3>Nenhuma GG disponível</h3></div>
            <?php else: ?>
            <div class="bins-grid">
                ...
                <div class="bin-card" onclick="showCards('<?php echo $bin['bin']; ?>')">
                    ...
                    <div>📦 Disponível: <?php echo $bin['total']; ?></div>
                </div>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
    ...
    <div class="modal" id="cardsModal">
        <div class="modal-content">
            <div class="modal-header"><h2 id="modalBinTitle">Cartões</h2><div class="modal-close" onclick="closeModal()">✖</div></div>
            <div id="cardsList"></div>
        </div>
    </div>

    <script>
        // Partículas e cursor (mesmo script anterior) - omitido por brevidade, mas deve ser incluso
        // (o código completo teria a função de partículas e cursor igual ao login)
        // Por economia de espaço, colocarei apenas o essencial:
                ...
                .then(cards=>{
                    ...
                    cards.forEach(c=>{
                        html += `<div class="card-item">
                            <div>🔗 BIN: ${c.bin}<br>📅 ${c.expiry.replace('|','/')}<br>💰 R$ ${c.price.toFixed(2)}</div>
                            <form method="POST">
                                <input type="hidden" name="purchase_gg" value="1">
                                <input type="hidden" name="gg_id" value="${c.id}">
                                <button type="submit" class="btn-buy" ${c.price > <?php echo $user_cyber; ?> ? 'disabled' : ''}>COMPRAR</button>
                            </form>
                        </div>`;
                    });
                    document.getElementById('modalBinTitle').innerText = `BIN ${bin}`;
...
<head>
    ...
    <style>
        * {
        ...
        .header {
            background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid #0f0; border-radius:40px; padding:30px; margin-bottom:30px; display:flex; align-items:center; justify-content:space-between; box-shadow:0 0 50px #0f0;
        }
        .header h1 { font-size:36px; color:#0f0; text-shadow:0 0 20px #0f0; }
        .nav { display:flex; gap:15px; }
        .nav-btn { padding:12px 25px; background:rgba(0,0,0,0.7); border:2px solid #0f0; border-radius:20px; color:#0f0; text-decoration:none; font-weight:bold; transition:all 0.3s; }
        .nav-btn:hover { background:#0f0; color:#000; transform:translateY(-5px); box-shadow:0 0 30px #0f0; }
        .stats-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:20px; margin-bottom:30px; }
        .stat-box { background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid #f0f; border-radius:30px; padding:20px; text-align:center; }
        .stat-value { font-size:42px; color:#ff0; text-shadow:0 0 20px #ff0; }
        .lives-grid { display:grid; gap:20px; }
        .live-card { background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid #0f0; border-radius:30px; padding:20px; }
        .live-header { display:flex; justify-content:space-between; border-bottom:2px solid #f0f; padding-bottom:10px; margin-bottom:15px; }
        .live-gate { background:linear-gradient(45deg,#0f0,#0ff); color:#000; padding:5px 20px; border-radius:20px; }
        .live-response { background:rgba(0,0,0,0.5); border:1px solid #0ff; border-radius:15px; padding:15px; font-family:monospace; white-space:pre-wrap; max-height:200px; overflow-y:auto; }
    </style>
</head>
<body>
    <canvas id="particle-canvas"></canvas>
    ...
    <div class="container">
        <div class="header">
            <div><h1>📋 MINHAS LIVES</h1><div style="color:#f0f;">👤 <?php echo $_SESSION['username']; ?></div></div>
            <div class="nav">
                <a href="<?php echo $_SERVER['PHP_SELF']; ?>" class="nav-btn">🏠 MENU</a>
        ...
        <?php if(empty($lives)): ?>
        <div style="text-align:center; padding:50px; background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border-radius:40px;"><div style="font-size:64px;">📭</div><h3>Nenhuma live encontrada</h3></div>
        <?php else: 
            $total = count($lives);
            $gates = count(array_unique(array_column($lives,'gate')));
            $last = $lives[0]['created_at'];
        ?>
        <div class="stats-grid">
            <div class="stat-box"><div>✅ TOTAL</div><div class="stat-value"><?php echo $total; ?></div></div>
            <div class="stat-box"><div>🔧 GATES</div><div class="stat-value"><?php echo $gates; ?></div></div>
            <div class="stat-box"><div>⏱️ ÚLTIMA</div><div class="stat-value" style="font-size:16px;"><?php echo date('d/m H:i',strtotime($last)); ?></div></div>
        </div>
        <div class="lives-grid">
            <?php foreach($lives as $live): ?>
            <div class="live-card">
                <div class="live-header"><span class="live-gate"><?php echo strtoupper($live['gate']); ?></span><span class="live-date"><?php echo date('d/m/Y H:i:s',strtotime($live['created_at'])); ?></span></div>
                <div class="live-bin">💳 BIN: <?php echo $live['bin']; ?></div>
                <div><strong>📱 Cartão:</strong> <?php echo substr($live['card'],0,6).'******'.substr($live['card'],-4); ?></div>
                <div class="live-response"><?php echo nl2br(htmlspecialchars($live['response'])); ?></div>
            </div>
            <?php endforeach; ?>
        </div>
        <?php endif; ?>
    ...
    <script>
        // Partículas (código igual ao anterior)
        const canvas = document.getElementById('particle-canvas');
        const ctx = canvas.getContext('2d');
        let width, height, particles = [];
        function init() {
            ...
            for(let i=0;i<100;i++) particles.push({
                ...
                vx: (Math.random()-0.5)*2, vy: (Math.random()-0.5)*2,
                size: Math.random()*3+1,
                color: `rgba(${Math.random()>0.5?'0,255,0':'255,0,255'},${Math.random()*0.5+0.2})`
            });
        }
        function draw() {
            ctx.clearRect(0,0,width,height);
...
if (isset($_GET['tool'])) {
    ...
    if (!isset($all_gates[$selected_tool])) { header('Location: '.$_SERVER['PHP_SELF']); exit; }
    if (!isGateActive($selected_tool)) { header('Location: '.$_SERVER['PHP_SELF'].'?error=gate_inactive'); exit; }
...
<head>
    ...
    <style>
        * {
        ...
        body {
            background:#000; color:<?php echo $gate['color']; ?>;
            min-height:100vh; padding:30px; position:relative; overflow-x:hidden;
        }
        #particle-canvas { position:fixed; top:0; left:0; width:100%; height:100%; z-index:0; }
        .grid-3d {
        ...
        .header {
            background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid <?php echo $gate['color']; ?>;
            border-radius:40px; padding:30px; margin-bottom:30px; display:flex; align-items:center; justify-content:space-between;
            box-shadow:0 0 50px <?php echo $gate['color']; ?>;
        }
        .header h1 { font-size:42px; display:flex; align-items:center; gap:15px; }
        .user-info { background:rgba(0,0,0,0.7); border:2px solid #f0f; border-radius:25px; padding:10px 25px; color:#f0f; box-shadow:0 0 20px #f0f; }
        .nav { display:flex; gap:15px; margin-bottom:30px; flex-wrap:wrap; justify-content:center; }
        .nav-btn { padding:12px 25px; background:rgba(0,0,0,0.7); border:2px solid <?php echo $gate['color']; ?>; border-radius:20px; color:<?php echo $gate['color']; ?>; text-decoration:none; font-weight:bold; transition:all 0.3s; cursor:pointer; }
        .nav-btn:hover { background:<?php echo $gate['color']; ?>; color:#000; transform:translateY(-5px); box-shadow:0 0 30px <?php echo $gate['color']; ?>; }
        .loading { display:none; align-items:center; gap:10px; color:#ff0; }
        .loading.active { display:flex; }
        .spinner { width:20px; height:20px; border:3px solid #ff0; border-top-color:transparent; border-radius:50%; animation:spin 1s linear infinite; }
        @keyframes spin { to { transform:rotate(360deg); } }
        textarea { width:100%; height:200px; background:rgba(0,0,0,0.7); border:2px solid <?php echo $gate['color']; ?>; border-radius:25px; color:<?php echo $gate['color']; ?>; padding:20px; font-family:monospace; margin-bottom:30px; }
        .stats-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:20px; margin-bottom:30px; }
        .stat-box { background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid <?php echo $gate['color']; ?>; border-radius:30px; padding:20px; text-align:center; }
        .stat-value { font-size:38px; color:#ff0; text-shadow:0 0 15px #ff0; }
        .results-grid { display:grid; grid-template-columns:1fr 1fr; gap:30px; }
        .result-box { background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid; border-radius:30px; padding:20px; max-height:500px; overflow-y:auto; }
        .result-box.live { border-color:#0f0; box-shadow:0 0 30px #0f0; }
        .result-box.die { border-color:#f00; box-shadow:0 0 30px #f00; }
        .result-item { background:rgba(0,0,0,0.5); border-left:6px solid; border-radius:15px; padding:15px; margin-bottom:15px; font-family:monospace; white-space:pre-wrap; }
        .result-item.live { border-left-color:#0f0; }
        .result-item.die { border-left-color:#f00; }
        .credits-counter, .cyber-counter { position:fixed; bottom:30px; right:30px; background:rgba(0,0,0,0.9); backdrop-filter:blur(10px); border:2px solid #f0f; border-radius:25px; padding:15px 30px; color:#f0f; font-weight:bold; z-index:100; box-shadow:0 0 30px #f0f; }
        .cyber-counter { left:30px; right:auto; border-color:#ff0; color:#ff0; }
    </style>
</head>
<body>
    <canvas id="particle-canvas"></canvas>
    ...
    <div class="container">
        ...
        <textarea id="dataInput" placeholder="Cole os cartões (numero|mes|ano|cvv)"></textarea>

        <div class="stats-grid">
            <div class="stat-box"><div class="stat-label">TOTAL</div><div class="stat-value" id="totalCount">0</div></div>
            <div class="stat-box"><div class="stat-label">✅ LIVES</div><div class="stat-value" id="liveCount">0</div></div>
            <div class="stat-box"><div class="stat-label">❌ DIES</div><div class="stat-value" id="dieCount">0</div></div>
    ...
    <script>
        // Partículas e cursor (código igual ao anterior, apenas adaptar)
        const canvas = document.getElementById('particle-canvas');
        const ctx = canvas.getContext('2d');
        let width, height, particles = [];
        function init() {
            ...
            for(let i=0;i<100;i++) particles.push({
                ...
                vx: (Math.random()-0.5)*2, vy: (Math.random()-0.5)*2,
                size: Math.random()*3+1,
                color: `rgba(${Math.random()>0.5?'255,255,255':'255,255,255'},${Math.random()*0.2})`
            });
        }
        function draw() {
            ctx.clearRect(0,0,width,height);
        ...
        const MAX_ITEMS = 200, DELAY = 4000;
        const LIVE_COST = <?php echo LIVE_COST; ?>;
        const DIE_COST = <?php echo DIE_COST; ?>;

        function checkIfLive(resp) {
            const patterns = ['✅','aprovada','approved','success','live','autorizado','authorized','valid','aprovado','apvd','ativa','active'];
            resp = resp.toLowerCase();
            return patterns.some(p => resp.includes(p.toLowerCase()));
        }

        function updateCounters() {
            if(document.getElementById('currentCredits')) document.getElementById('currentCredits').textContent = currentCredits.toFixed(2);
        ...
        function startCheck() {
            ...
            items = input.split('\n').filter(l=>l.trim());
            if(items.length>MAX_ITEMS) { alert(`⚠️ Máx ${MAX_ITEMS}`); items = items.slice(0,MAX_ITEMS); }
            currentIndex = 0;
            isChecking = true;
        ...
        async function processNext() {
            ...
            try {
                ...
                if(userType==='credits') {
                    const cost = isLive ? LIVE_COST : DIE_COST;
                    currentCredits -= cost;
            ...
            if(isChecking && currentIndex<items.length) setTimeout(processNext, DELAY);
            else stopCheck();
        }

        function addResult(item, resp, isLive) {
            ...
            div.innerHTML = `<strong>📱 ${item}</strong><br><br>${resp.replace(/\n/g,'<br>')}`;
...
// ============================================
$gates_config = loadGatesConfig();
$active_gates = array_filter($gates_config);
...
<head>
    ...
    <style>
        * {
        ...
        body {
            background:#000; color:#0ff; min-height:100vh; padding:30px; position:relative; overflow-x:hidden;
        }
        #particle-canvas { position:fixed; top:0; left:0; width:100%; height:100%; z-index:0; }
        .grid-3d {
            position:fixed; width:200%; height:200%;
            background-image: linear-gradient(rgba(0,255,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(255,0,255,0.03) 1px, transparent 1px);
        ...
        .container { max-width:1400px; margin:0 auto; position:relative; z-index:10; }
        .header {
            background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid #f0f; border-radius:60px; padding:70px 60px; margin-bottom:40px; text-align:center; box-shadow:0 0 50px #f0f;
        }
        .header h1 { font-size:82px; color:#0ff; text-shadow:0 0 40px #0ff,0 0 80px #f0f; margin-bottom:20px; animation:glitch 3s infinite; }
        @keyframes glitch {
            0%,100%{transform:skew(0);}
            95%{transform:skew(5deg,-2deg); text-shadow:5px 0 #f0f,-5px 0 #0ff;}
            96%{transform:skew(-5deg,2deg); text-shadow:-5px 0 #f0f,5px 0 #0ff;}
        }
        .header p { color:#f0f; font-size:20px; letter-spacing:10px; }
        .user-info { position:absolute; top:30px; right:30px; background:rgba(0,0,0,0.7); border:2px solid #0ff; border-radius:30px; padding:15px 30px; display:flex; align-items:center; gap:15px; }
        .status-bar { display:flex; justify-content:center; gap:40px; margin-bottom:40px; flex-wrap:wrap; }
        .status-item { background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid; border-radius:40px; padding:30px 60px; text-align:center; min-width:280px; transition:all 0.3s; }
        .status-item:hover { transform:scale(1.1); box-shadow:0 0 40px currentColor; }
        .status-credits { border-color:#f0f; color:#f0f; }
        .status-cyber { border-color:#ff0; color:#ff0; }
        .status-gates { border-color:#0ff; color:#0ff; }
        .status-value { font-size:42px; font-weight:bold; text-shadow:0 0 20px currentColor; }
        .nav { display:flex; gap:25px; margin-bottom:60px; justify-content:center; flex-wrap:wrap; }
        .nav-btn { padding:15px 40px; background:rgba(0,0,0,0.7); border:2px solid #0ff; border-radius:30px; color:#0ff; text-decoration:none; font-weight:bold; transition:all 0.3s; }
        .nav-btn:hover { background:#0ff; color:#000; transform:translateY(-5px); box-shadow:0 20px 50px #0ff; }
        .ggs-badge { background:#f0f; color:#000; padding:4px 12px; border-radius:20px; margin-left:5px; }
        .gates-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(320px,1fr)); gap:30px; margin-top:30px; }
        .gate-card { background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid; border-radius:40px; padding:30px; text-decoration:none; color:inherit; transition:all 0.3s; position:relative; }
        .gate-card:hover { transform:translateY(-15px); box-shadow:0 30px 60px rgba(0,255,255,0.4); }
        .gate-card.inactive { opacity:0.4; filter:grayscale(1); pointer-events:none; }
        .gate-icon { font-size:72px; text-align:center; margin-bottom:20px; }
        .gate-card h3 { color:#0ff; text-align:center; font-size:24px; }
        .gate-status { position:absolute; top:20px; right:20px; width:20px; height:20px; border-radius:50%; }
        .status-active { background:#0f0; box-shadow:0 0 30px #0f0; animation:pulse 1.5s infinite; }
        .status-inactive { background:#f00; box-shadow:0 0 30px #f00; }
        @keyframes pulse { 0%,100%{opacity:1;} 50%{opacity:0.5;} }
        .info-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(280px,1fr)); gap:30px; margin-top:60px; }
        .info-card { background:rgba(10,20,30,0.3); backdrop-filter:blur(20px); border:2px solid #f0f; border-radius:40px; padding:35px; text-align:center; transition:all 0.3s; }
        .info-card:hover { transform:translateY(-10px); box-shadow:0 0 40px #f0f; }
        .info-icon { font-size:56px; margin-bottom:20px; }
        .info-value { font-size:32px; color:#ff0; text-shadow:0 0 20px #ff0; }
        .custom-cursor { width:20px; height:20px; border:2px solid #0ff; border-radius:50%; position:fixed; pointer-events:none; z-index:9999; mix-blend-mode:difference; transition:transform 0.1s; }
    </style>
</head>
<body>
    <canvas id="particle-canvas"></canvas>
    ...
    <div class="container">
        ...
        <div class="status-bar">
            ...
            <div class="status-item status-credits"><div class="status-label">💰 CRÉDITOS</div><div class="status-value"><?php echo number_format($user_credits,2); ?></div></div>
            <?php endif; ?>
            <div class="status-item status-cyber"><div class="status-label">🪙 MOEDAS CYBER</div><div class="status-value"><?php echo number_format($user_cyber,2); ?></div></div>
            <div class="status-item status-gates"><div class="status-label">🔧 GATES ATIVAS</div><div class="status-value"><?php echo count($active_gates); ?>/<?php echo count($all_gates); ?></div></div>
        </div>

        <div class="nav">
            <a href="?ggs" class="nav-btn">🛒 LOJA GG <?php if($ggs_available>0): ?><span class="ggs-badge"><?php echo $ggs_available; ?></span><?php endif; ?></a>
            <a href="?lives" class="nav-btn">📋 MINHAS LIVES</a>
            <?php if($user_role==='admin'): ?><a href="?admin=true" class="nav-btn">⚙ PAINEL ADMIN</a><?php endif; ?>
            <a href="?logout" class="nav-btn">🚪 SAIR</a>
        </div>

        <h2 style="color:#f0f; margin-bottom:20px; text-align:center;">🔧 CHECKERS DISPONÍVEIS</h2>
        <div class="gates-grid">
        ...
        <div class="info-grid">
            <div class="info-card"><div class="info-icon">📊</div><div class="info-title">TOTAL DE CHECKS</div><div class="info-value"><?php echo $current_user['total_checks']??0; ?></div></div>
            <div class="info-card"><div class="info-icon">✅</div><div class="info-title">TOTAL DE LIVES</div><div class="info-value"><?php echo $current_user['total_lives']??0; ?></div></div>
            <div class="info-card"><div class="info-icon">📅</div><div class="info-title">MEMBRO DESDE</div><div class="info-value"><?php echo isset($current_user['created_at'])?date('d/m/Y',strtotime($current_user['created_at'])):date('d/m/Y'); ?></div></div>
            <div class="info-card"><div class="info-icon">🔐</div><div class="info-title">ÚLTIMO ACESSO</div><div class="info-value"><?php echo isset($current_user['last_login'])?date('d/m H:i',strtotime($current_user['last_login'])):'Primeiro'; ?></div></div>
        </div>
    </div>
    ...
    <script>
        ...
        function init() {
            ...
            for(let i=0;i<150;i++) particles.push({
                ...
                color: `rgba(${Math.random()>0.5?'0,255,255':'255,0,255'},${Math.random()*0.3+0.1})`
            });
        }
        function draw() {
            ctx.clearRect(0,0,width,height);
