...
// Array de códigos LIVE
$live_codes = ['N7', '51', '63', '83', '12', 'VBV', 'FA', 'A6', '100', '101'];
...
if ($http_code == 200 && (!isset($response_data['error']) || $response_data['error'] === false || isset($response_data['success']))) {
    ...
    // Verificar se o código está na lista de LIVE
    if (in_array($codigo, $live_codes)) {
        // Salvar como LIVE (mesmo arquivo dos aprovados)
        file_put_contents('cx2dorainzinho.txt', $cartao_formatado . ' | LIVE CODE: ' . $codigo . PHP_EOL, FILE_APPEND);
        echo '<span class="badge badge-success">💎 LIVE</span> ' . $cartao_formatado . ' | <span class="transation badge-success">Código: ' . $codigo . ' (LIVE)</span>';
    } else {
        echo '<span class="badge badge-danger">❌ REPROVADO</span> ' . $cartao_formatado . ' | <span class="transation badge-danger">' . $mensagem . ($codigo ? ' (' . $codigo . ')' : '') . '</span>';
    }
}
